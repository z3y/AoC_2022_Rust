use std::{fs::File, io::{BufReader, BufRead}, cmp::{max, min}};
    ...
    pub fn get_total_size(&self) -> i32 {
        ...
        while i < self.nodes.len() {
            match &self.nodes[i] {
                ...
                Node::File { name: _, size } => sum += size,
            }
            i+=1;
        }
        sum
...
fn main()  {
    ...
    let total_size = tree.get_total_size();
    ...
    println!("required free space {}", req_space);
    println!("deleting {}", smallest);
    println!("resulting free space {}", free_space + smallest);
